...
from data.Frihand.Frihand_loader import get_Frihand_loader
...
def train(train_func, config, post_epoch=None):
    while True:
        ...
        for phase in ['train', 'valid']:
            ...
            generator = get_Frihand_loader(config['train']['batchsize'], period=phase)
            ...
            for i in show_range:
            # for i in range(2):
                imgs, hms = next(generator)
                datas = {}
                datas['imgs'] = imgs.permute(0,2,3,1) # B,C,H,W --> B,H,W,C
                datas['heatmaps'] = hms
                # print('---------data--------')
                # print(datas['imgs'].shape)
                # print(datas['heatmaps'].shape)
